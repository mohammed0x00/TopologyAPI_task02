...
TEST(API, QueryDevicesAPITest) {
	...
	EXPECT_TRUE((string)(API::queryDevices("topology2").list[0]->getID()) == "res2");
	API::queryDevices("topology2").list[0]->setID((Device::DeviceIdType)"res3");
	EXPECT_TRUE((string)(API::queryDevices("topology2").list[0]->getID()) == "res3");
}

TEST(API, DeviceListSizeTest) {
	/* Query Devices in "topology2" and read number of devices */
	EXPECT_EQ(API::queryDevices("topology2").size, 2);
}

TEST(API, QueryDevicesWithNetlistNode) {
	/* Query Devices in "topology2" that are connected to "vin", read a device ID */
	EXPECT_TRUE((string)(API::queryDevicesWithNetlistNode("topology2", "vin").list[0]->getID()) == "m1");
}

TEST(API, WriteJSONTest) {
	/* Write topology "topology2" to a .json file */
	EXPECT_EQ(API::writeJSON((Topology::TopologyIdType)"topology2"), API::Done);
	
	/* Test if the file was written correctly */
	try {
	        ifstream in1("topology2.json", ios::binary);
